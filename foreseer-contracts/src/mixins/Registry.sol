...
abstract contract Registry is IRegistry {
    ...
    function _registerToken(uint256 token0, uint256 token1, bytes32 conditionId, address amm) internal {
        ...
        registry[token0] = OutcomeToken({complement: token1, conditionId: conditionId, amm: amm});

        registry[token1] = OutcomeToken({complement: token0, conditionId: conditionId, amm: amm});

        emit TokenRegistered(token0, token1, conditionId, amm);
        emit TokenRegistered(token1, token0, conditionId, amm);
    }
}
